...
public class PaySuccess3DSTestsBTV {
    @Test
    void PayTest3DSBTV() {
        ...
        $x("(//input[@class='input-component__input'])[1]").sendKeys("2200000000000004");
        $x("(//input[@id='date'])[1]").sendKeys("0126");
        $x("(//input[@id='cvv'])[1]").sendKeys("111");
        $x("(//button[@class='button button_primary'])[1]").click();
        SelenideElement iframePayment2 = $x("//iframe[@class='threeDs_frame loaded']");
        switchTo().frame(iframePayment2);
        $x("(//button[@class='button button_primary'])").click();
        switchTo().frame(0);
